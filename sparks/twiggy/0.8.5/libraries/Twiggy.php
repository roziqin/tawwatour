...
class Twiggy
{
	...
	*/
	...
	public function __construct()
	{
		...
		if(!empty($this->_config['default_public_theme'])) {
			$public_theme = 'public_themes/'.$this->_config['default_public_theme'];
			$this->_set_template_locations($public_theme);
			try
			{
				$this->_twig_loader = new Twig_Loader_Filesystem($this->_template_locations);
				$this->publicTheme($this->_config['default_public_theme']);
			}
		...
		} else if(!empty($this->_config['default_admin_theme'])) {
			$admin_theme = 'admin_themes/'.$this->_config['default_admin_theme'];
			$this->_set_template_locations($admin_theme);
			try
			{
				$this->_twig_loader = new Twig_Loader_Filesystem($this->_template_locations);
				$this->adminTheme($this->_config['default_admin_theme']);
			}
			catch(Twig_Error_Loader $e)
			{
				log_message('error', 'Twiggy: failed to load the default theme');
			...
			log_message('error', 'Twiggy: failed to load the default theme');
			show_error("ERROR! failed to load default theme.");
		}
	...
	*/       	
	...
	public function adminTheme($theme)
	{
		if(empty($theme)) $theme = $this->_config['default_admin_theme'];

		$theme = 'admin_themes/'.$theme;
		if(!is_dir(realpath($this->_themes_base_dir. $theme)))
		{
			log_message('error', 'Twiggy: requested admin theme '. $theme .' has not been loaded because it does not exist.');
			show_error("Theme does not exist in {$this->_themes_base_dir}{$theme}.");
		}
	...
	public function publicTheme($theme)
	{

		if(empty($theme)) $theme = $this->_config['default_public_theme'];

		$theme = 'public_themes/'.$theme;
		if(!is_dir(realpath($this->_themes_base_dir. $theme)))
		{
			log_message('error', 'Twiggy: requested public theme '. $theme .' has not been loaded because it does not exist.');
			show_error("Theme does not exist in {$this->_themes_base_dir}{$theme}.");
		}
