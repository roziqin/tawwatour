...
class AdminBlogCategories extends XE_Controller {

    function __construct()
    {
        parent::__construct('blog-category-admin');
    ...
    public function index()
    {
        ...
        $data['categorys'] = MDBlogCategory::all(array('conditions'=>'deleted = 0','order'=>'name'));
        $this->viewData($data);
        $this->displayView('blogs_category_index');
    }

    public function detail($category_id)
    {
        $data['message'] = '';
        $category = MDBlogCategory::find_by_id_and_deleted($category_id,0);
        if(!$category) {
            redirect(generate_url('blogs_category_index'));
        }
        $data['category'] = $category;
        $this->viewData($data);
        $this->displayView('blogs_category_detail');
    }
    ...
    public function form()
    {
        ...
        if(!empty($post)) {
            ...
            $this->form_validation->set_rules('blog_category_name', 'Name', 'required');
            if ($this->form_validation->run() == FALSE) {
                $message = err_msg(validation_errors());
            } else {
                
                $text = $post['blog_category_name'];
                ...
                $data_category = [
                    'name' => $post['blog_category_name'],
                    'alias' => $text
                ];
                $category_temp = MDBlogCategory::find_by_name_and_deleted($post['blog_category_name'],0);
         ...
         if($mode == 'edit') {
            ...
            if($form_submitted) {
                $res = MDBlogCategory::create($data_category);
                if($res) {
        ...
        $this->viewData($data);
        $this->displayView('blogs_category_form');
    }
}
