...
class XE_Controller extends MX_Controller {
	...
	function __construct($need_permission = false)
	{
		...
		$this->theme = $this->config->item('xe_default_theme');
		$this->twiggy->adminTheme($this->theme);
	...
	protected function viewTheme($set_theme='')
	{
		if(!empty($set_theme)) 
		{
			...
			$this->twiggy->adminTheme($set_theme);
		}
		return $this->theme;
	}
